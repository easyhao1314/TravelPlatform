...
public class xingchengkushituController {
	...
	@RequestMapping("fenghuang/xianluSelect.do")
	@ResponseBody
	public Map<String, Object> xianluSelect(HttpServletRequest request,
			HttpServletResponse response, Integer page,Integer rows,String xianid){
		Xianlu xl=new Xianlu();
		xl.setXianid(Long.parseLong(xianid));
		try {
			Pagination<Xianlu> pagination = is.xianlu(page, rows, xl);
			List<Map<String, Object>> testUsers = pagination.getResultList();
			Map<String,Object> returnValue  = new HashMap<String, Object>();
			for(int i = 0 ;i<testUsers.size();i++){
				for(Entry<String, Object> entry : testUsers.get(i).entrySet()){
