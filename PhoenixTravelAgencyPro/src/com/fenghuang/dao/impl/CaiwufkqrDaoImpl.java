...
public class CaiwufkqrDaoImpl extends BaseDao implements IcaiwufkqrDao {
	...
	public Pagination<Tuanbiao> getPaginationfkqr(int currentPage,
		...
		// TODO Auto-generated method stub
		StringBuffer sql=new StringBuffer("SELECT  users.userName as xiaoshouyuan,ss.userName,t.*,t.yingshou*b.huilv as syingshou,t.yishou*b.huilv as syishou,t.yfk*b.huilv as syfk,t.yifu*b.huilv as syifu,f.cashier,f.confirmed,f.review,f.`status`,b.bizhong,b.huilv+b.huilv-b.huilv as huilv,sanpinzhongxin.tuanName as sanpintuanName,dantuan.tuanName AS dantuanName,yfk*huilv-yifu*huilv as weifu,yingshou*huilv-yishou*huilv as ysweifu FROM tuanbiao AS t LEFT JOIN sanpinzhongxin ON t.tuanduimc=sanpinzhongxin.tuanNo LEFT JOIN finance AS f on t.caiwuid=f.id LEFT JOIN bizhonghuilv AS b on t.huilvID=b.id LEFT JOIN users AS ss ON ss.id=t.caozuo  LEFT JOIN dantuanxinxi as dantuan on t.tuanduimc=dantuan.tuanNO LEFT JOIN users on t.xiaoshou=users.id where t.shanchu=1 ");
