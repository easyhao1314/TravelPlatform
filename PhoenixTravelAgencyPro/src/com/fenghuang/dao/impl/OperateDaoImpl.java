...
public class OperateDaoImpl extends BaseDao implements IOperateDao {
	...
	public Pagination<Operate> OperateInfo(int page,int rows,Operate o) {
		...
		String sql = "select o.*,s.userName as jiedan ,ss.userName AS paidan,xian.xlid AS xianid,daoyou.`name` AS daoyouname from Operate as o LEFT JOIN users as s ON s.id=o.jiedanren LEFT JOIN users as ss ON ss.id=o.paidanren INNER JOIN tuanxianlu as xian ON xian.tuanNo=o.tuanNo LEFT JOIN daoyou ON daoyou.id=o.daoyou   where 1=1  ";
		StringBuffer sb = new StringBuffer(sql);
		if(o.getTuanNo()!=null && !"".equals(o.getTuanNo())){
			sb.append(" and o.tuanNo='");
